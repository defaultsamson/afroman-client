...
public class GuiYesNoPrompt extends GuiTextButton
{
	...
	public GuiYesNoPrompt(GuiScreen screen, int response, String text1, String text2)
	{
		...
		drawWidth = Math.max((2 * (BUTTON_WIDTH + BUTTON_SPACING)) + SIDE_SPACING, Math.max(font.getWidth(text1), font.getWidth(text2)) + SIDE_SPACING);
	...
	public void render(Texture drawTo)
	{
		...
		font.renderCentered(drawTo, drawLocation.getX() + (drawWidth / 2), drawLocation.getY() + 7, notifText1);
		font.renderCentered(drawTo, drawLocation.getX() + (drawWidth / 2), drawLocation.getY() + 19, notifText2);
